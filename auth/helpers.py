"""Shared helper functions for auth module."""
...
def _nested_value(fields, *path):
    """Extract nested value from dict using path keys."""
    current = fields
    for key in path:
...
def _items_from_payload(fields):
    """Extract items list from various payload formats."""
...
def _first_item(fields):
    """Get first item from payload, or empty dict."""
...
def _item_requested_cost(item):
    """Calculate requested cost for a single item."""
    amount = (
...
def _total_requested_cost(fields):
    """Calculate total requested cost from payload."""
    if not isinstance(fields, dict):
        return None
...
def _has_dashboard_data(fields):
    """Check if fields contain meaningful dashboard data."""
    if not isinstance(fields, dict):
        return False
...
def _dashboard_org_id(claims):
    """Get org_id from claims."""
    return claims["org_id"]


async def is_platform_admin(claims):
    """Check if user is a SaaSPro platform admin.
...
async def _resolve_read_org_id(claims: dict, requested_org_id: int | None = None) -> int:
    """Resolve org_id for read operations, checking platform admin access."""
...
async def _resolve_mutation_org_id(claims: dict, requested_org_id: int | None = None) -> int:
    """Resolve org_id for mutation operations, checking admin role."""
    if claims.get("role") != "admin":
...
async def _preauth_dashboard_org_id(claims):
    """Get org_id with fallback to busiest org (legacy helper)."""
...
# ─────────────────────────────────────────────
# Dashboard Row Formatting
# ─────────────────────────────────────────────

def _dashboard_request(row):
    """Transform a database row into a dashboard request dict."""
